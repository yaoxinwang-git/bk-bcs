...
func NewHandlerWrapper() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		name := fmt.Sprintf("%s.%s", c.Request.RequestURI)

		//tracer := otel.Tracer("bcs-monitor-api")
		commonAttrs := []attribute.KeyValue{
			attribute.String("component", c.Request.Proto),
			attribute.String("method", c.Request.Method),
			attribute.String("url", c.Request.URL.RequestURI()),
		}
